//This program will prompt the user to input 3 pieces of information to calculate the file size of an uncompressed image.  This can be useful to get a rough idea of how large the file for an image on a website could be.  This will multiply the length, width, and bit depth of the image.  It will then divide that number by 8192 to give the answer in KB.
...
//Calculations
fileSize = (horizontalPixels * vertPixels * bitDepth) / kbConvert; //multiplies horizontal pixels, vertical pixels, and bit depth and divides by 8192 to give us the number of kilobytes

//Output
console.log("The file size of this image is " + fileSize + "KB."); //Outputs the file size to the user
(fileSize < 200) ? console.log("This is a good size for a web image") : console.log("Your image is too large for the internet. Please reduce the size and try again"); //Makes a recommendation to the user based on file size to tell them if the file is a good size or should be reduced
